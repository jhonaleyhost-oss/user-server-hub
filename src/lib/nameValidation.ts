// Shared display-name validator used by profile updates and the register flow.
// Keep this in sync with supabase/functions/_shared/nameValidation.ts.
...
// Domain / URL / social-handle patterns that we never want as a display name.
const LINK_PATTERNS: RegExp[] = [
  ...
  /wa\.me/i,
  /\bchat\.whatsapp\b/i,
  // Any word ending in a common TLD, e.g. "foo.uk", "bar.com"
...
export function validateDisplayName(rawName: string): { ok: true } | { ok: false; error: string } {
  ...
  // Block control chars & emoji-heavy garbage; allow letters/numbers/space and . _ -
  if (!/^[A-Za-z0-9 ._-]+$/.test(name)) {
    return { ok: false, error: "Nama hanya boleh huruf, angka, spasi, titik, garis bawah, dan strip." };
  }

  // No leading/trailing dot or dash, no consecutive spaces
  if (/^[.\-_ ]|[.\-_ ]$/.test(name)) {
  ...
  // Reserved / impersonation words (whole-word match, case-insensitive)
